...
import {
  mdiEye, mdiArrowLeft, mdiNote, mdiImageArea, mdiVideo, mdiFileDocument,mdiAccountTie,
  mdiViewModule, mdiFileDocumentEdit
} from '@mdi/js';
import { formatDate, areDatesDifferent } from '../../utils/dates';

function ViewEnrolledModule() {
  ...
  const renderTypeIcon = (type) => {
    ...
    if (t === 'assessment') return <Icon path={mdiFileDocumentEdit} size={1.2} />;
      ...
      <div className="card mb-4">
        ...
        <div className="card-footer bg-white border-0 d-flex justify-content-between align-items-center">
          ...
          <button 
            ...
            onClick={() => navigate(`/TakeAssessment/${courseId}/`)}
          >
            <Icon path={mdiFileDocumentEdit} size={0.8} /> Take Assessment
          </button>
        </div>
      </div>
              ...
              <div className="d-flex gap-2" style={{ marginTop: '8px' }}>
                {content.mediaUrl && (
                  <button
                    ...
                    onClick={() => navigate(`/${content.courseContentId}/ViewContent`)}
                    >
                    <Icon path={mdiEye} size={0.8} /> View Content
                </button>
                )}
...
export default ViewEnrolledModule;
